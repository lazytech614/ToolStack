...
import { EXAMPLE_TEXT } from "@/constants/configs/examples";
...
function HashRow({ algo, value }: { algo: Algorithm; value: string | undefined }) {
  ...
  return (
    <div className="flex flex-col gap-2">
        ...
        <button
          ...
          className={cn(
            ...
            copied
              ...
              : "cursor-pointer text-zinc-500 hover:text-zinc-900 disabled:cursor-default disabled:opacity-40 dark:text-zinc-400 dark:hover:text-white",
          )}
        >
      ...
      <pre className="min-h-8 rounded-xl border border-zinc-200 bg-zinc-50 px-4 py-3 font-mono text-xs break-all whitespace-pre-wrap text-zinc-900 dark:border-zinc-800 dark:bg-zinc-900 dark:text-zinc-100">
...
export function HashGenerator() {
  ...
  // file tab
  const [fileName, setFileName] = useState(EXAMPLE_TEXT);
  ...
  // compare tab
  const [compareA, setCompareA] = useState(EXAMPLE_TEXT);
  const [compareB, setCompareB] = useState(EXAMPLE_TEXT);
  ...
  useEffect(() => {
    ...
    setComputing(true);
    Promise.all(ALGORITHMS.map(async (a) => [a, await hashText(a, input)] as const)).then(
      (entries) => {
  ...
  function handleReset() {
    ...
    setFileName(EXAMPLE_TEXT);
    setCompareA(EXAMPLE_TEXT);
    setCompareB(EXAMPLE_TEXT);
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      <div className="flex items-center justify-between">
        ...
        <button
          onClick={handleReset}
          className="flex cursor-pointer items-center gap-1.5 px-2.5 text-xs font-medium text-zinc-500 transition-colors hover:text-zinc-900 dark:text-zinc-400 dark:hover:text-white"
        >
        ...
        <>
          <div className="flex flex-col gap-2">
            ...
            <div
              ...
              className="flex cursor-pointer flex-col items-center justify-center gap-3 rounded-xl border-2 border-dashed border-zinc-200 bg-zinc-50 px-6 py-12 text-center transition-colors hover:border-purple-400 hover:bg-purple-50/40 dark:border-zinc-800 dark:bg-zinc-900 dark:hover:border-purple-500/60 dark:hover:bg-purple-500/5"
            >
              ...
              <div className="flex flex-col gap-2">
                ...
                {[compareA.trim().toLowerCase(), compareB.trim().toLowerCase()].map((h, hi) => {
                  ...
                  return (
                    <pre
                      key={hi}
                      className="rounded-xl border border-zinc-200 bg-zinc-50 px-4 py-3 font-mono text-xs break-all whitespace-pre-wrap dark:border-zinc-800 dark:bg-zinc-900"
                    >
                      {h.split("").map((ch, ci) => (
                        <span
                          key={ci}
                          className={cn(
                            other[ci] !== ch
                              ? "rounded bg-red-50 text-red-500 dark:bg-red-500/10 dark:text-red-400"
