...
import { EXAMPLE_MARKDOWN_TABLE } from "@/constants/configs/examples";
...
import {
  ...
  parseMarkdown,
  renderTableHTML,
} from "@/lib/dev-utils/markdown-table";
...
export function MarkdownTable() {
  ...
  // ── Example ──
  const handleExample = () => setTable(EXAMPLE_MARKDOWN_TABLE);
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <div className="relative">
          <select
            ...
            className="cursor-pointer appearance-none rounded-md border border-zinc-200 bg-zinc-50 px-3 py-1.5 pr-7 text-xs font-medium text-zinc-700 focus:ring-2 focus:ring-purple-500/40 focus:outline-none dark:border-zinc-800 dark:bg-zinc-900 dark:text-zinc-300"
          >
        ...
        <div className="flex items-center gap-2 sm:ml-auto">
          <button
            ...
            className="cursor-pointer text-xs font-medium text-zinc-500 transition-colors hover:text-zinc-900 dark:text-zinc-400 dark:hover:text-zinc-200"
          >
            Try example
          </button>
          <button
      ...
      {/* ── Mobile tabs ── */}
      <div className="flex gap-1 rounded-lg border border-zinc-200 p-1 lg:hidden dark:border-zinc-800">
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
        ...
        <div
          className={cn(
            "flex flex-col gap-3 lg:col-span-1",
        ...
        >
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-1">
              <button
                onClick={() => addRow()}
                title="Add row"
                className="flex items-center gap-0.5 rounded px-1.5 py-1 text-xs text-zinc-500 transition-colors hover:bg-zinc-100 hover:text-purple-600 dark:text-zinc-400 dark:hover:bg-zinc-800 dark:hover:text-purple-400"
              >
            ...
            <table className="w-full border-collapse text-xs">
              ...
              <thead>
                <tr className="bg-zinc-50 dark:bg-zinc-900">
                  ...
                  {table.headers.map((_, ci) => (
                    <td
                      key={ci}
                      className="border-b border-zinc-200 px-1 py-1 dark:border-zinc-800"
                    >
                      <div className="flex items-center justify-between gap-0.5">
                        ...
                        <div className="flex gap-0.5">
                          <button
                            ...
                            disabled={ci === 0}
                            className="p-0.5 text-zinc-400 transition-colors hover:text-zinc-700 disabled:opacity-20 dark:hover:text-zinc-200"
                          >
                          ...
                          <button
                            ...
                            disabled={cols <= 1}
                            className="p-0.5 text-red-400 transition-colors hover:text-red-600 disabled:opacity-20"
                          >
                ...
                <tr className="bg-zinc-50 dark:bg-zinc-900/80">
                  <td className="border-b border-zinc-200 px-1 text-center font-mono text-[10px] text-zinc-300 dark:border-zinc-800 dark:text-zinc-700">
                    #
                  </td>
                  {table.headers.map((h, ci) => (
                    <td
                      key={ci}
                      className="border-b border-l border-zinc-200 p-0 dark:border-zinc-800"
                    >
                      <input
                        value={h}
                        onChange={(e) => updateHeader(ci, e.target.value)}
                        placeholder={`Col ${ci + 1}`}
                        className="w-full min-w-20 bg-transparent px-2 py-2 font-semibold text-zinc-900 placeholder:text-zinc-300 focus:bg-purple-50 focus:outline-none dark:text-zinc-100 dark:placeholder:text-zinc-700 dark:focus:bg-purple-900/10"
                  ...
                  <td className="border-b border-l border-zinc-200 dark:border-zinc-800">
                    <button
                      onClick={() => addCol()}
                      className="flex h-full w-full items-center justify-center p-2 text-zinc-300 transition-colors hover:text-purple-600 dark:text-zinc-700 dark:hover:text-purple-400"
                    >
              ...
              <tbody>
                {table.rows.map((row, ri) => (
                  ...
                  >
                    {/* Row controls */}
                    <td className="border-b border-zinc-100 px-1 dark:border-zinc-800/60">
                      <div className="flex flex-col items-center gap-0.5 opacity-0 transition-opacity group-hover:opacity-100">
                        <button
                          ...
                          disabled={ri === 0}
                          className="text-zinc-400 hover:text-zinc-700 disabled:opacity-20 dark:hover:text-zinc-200"
                        >
                    ...
                    {row.map((cell, ci) => (
                      <td
                        key={ci}
                        className="border-b border-l border-zinc-100 p-0 dark:border-zinc-800/60"
                      >
                        <input
                          ref={(el) => {
                          ...
                          placeholder="—"
                          className="w-full min-w-20 bg-transparent px-2 py-2 text-zinc-700 placeholder:text-zinc-200 focus:bg-purple-50 focus:outline-none dark:text-zinc-300 dark:placeholder:text-zinc-800 dark:focus:bg-purple-900/10"
                          style={{ textAlign: table.alignments[ci] }}
                        />
                      </td>
                    ))}
                    ...
                    <td className="border-b border-l border-zinc-100 dark:border-zinc-800/60">
                      <div className="flex flex-col items-center gap-0.5 p-0.5 opacity-0 transition-opacity group-hover:opacity-100">
                        <button
                          onClick={() => duplicateRow(ri)}
                          title="Duplicate row"
                          className="p-0.5 text-zinc-400 transition-colors hover:text-purple-600 dark:hover:text-purple-400"
                        >
                ...
                <tr>
                  ...
                  <td colSpan={cols + 1} className="p-0">
                    <button
                      onClick={() => addRow()}
                      className="flex w-full items-center justify-center gap-1 py-2 text-xs text-zinc-300 transition-colors hover:bg-zinc-50 hover:text-purple-600 dark:text-zinc-700 dark:hover:bg-zinc-900/30 dark:hover:text-purple-400"
                    >
          ...
          <div className="flex flex-col gap-2">
            ...
            <textarea
              ...
              rows={5}
              className="w-full resize-none rounded-lg border border-zinc-200 bg-zinc-50 p-3 font-mono text-xs text-zinc-900 placeholder:text-zinc-400 focus:ring-2 focus:ring-purple-500/40 focus:outline-none dark:border-zinc-800 dark:bg-zinc-900 dark:text-zinc-100 dark:placeholder:text-zinc-600"
            />
            {importError && <p className="text-xs text-red-500 dark:text-red-400">{importError}</p>}
            <button
              onClick={importMarkdownTable}
              disabled={!markdownInput.trim()}
              className="self-start rounded-md bg-zinc-100 px-3 py-1.5 text-xs font-medium text-zinc-700 transition-colors hover:bg-zinc-200 disabled:cursor-not-allowed disabled:opacity-40 dark:bg-zinc-800 dark:text-zinc-300 dark:hover:bg-zinc-700"
            >
              Import Markdown
            </button>
          </div>
        ...
        >
          ...
          <div className="min-h-50 flex-1 overflow-auto rounded-lg border border-zinc-200 bg-zinc-50 p-3 dark:border-zinc-800 dark:bg-zinc-900">
            <pre className="font-mono text-xs leading-relaxed whitespace-pre text-zinc-700 dark:text-zinc-300">
              {markdown}
            </pre>
          </div>
        </div>
        ...
        >
          ...
          <div className="min-h-50 flex-1 overflow-auto rounded-lg border border-zinc-200 bg-white p-4 dark:border-zinc-800 dark:bg-zinc-950">
            <style>{`
            ...
            `}</style>
            <table className="md-preview-table" dangerouslySetInnerHTML={{ __html: previewHTML }} />
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-zinc-400 dark:text-zinc-600">
        ...
        <span className="ml-auto hidden opacity-60 sm:block">
          Tab / Shift+Tab to navigate · Enter to move down · Paste CSV directly into cells
        </span>
      </div>
    </div>
  );
