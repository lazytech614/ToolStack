...
import { Copy, Check, Download, RefreshCw, ChevronDown } from "lucide-react";
...
function CopyButton({ text }: { text: string }) {
  ...
  return (
    <button
      onClick={handle}
      disabled={!text}
      className="rounded p-1 text-zinc-400 transition-colors hover:text-zinc-700 disabled:opacity-30 dark:hover:text-zinc-200"
    >
...
function BitVisualizer({ binary }: { binary: string }) {
  ...
  return (
    <div className="mt-2 flex flex-wrap gap-1">
      {isNeg && (
        <span className="mr-1 self-center font-mono text-[10px] text-red-500 dark:text-red-400">
          −
        </span>
      )}
      {clean.split("").map((bit, i) => (
        <span
          key={i}
          className={cn(
            "inline-flex h-6 w-6 items-center justify-center rounded border font-mono text-[11px] font-semibold transition-colors",
            bit === "1"
              ? "border-purple-300 bg-purple-100 text-purple-700 dark:border-purple-700 dark:bg-purple-900/40 dark:text-purple-300"
              : "border-zinc-200 bg-zinc-100 text-zinc-400 dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-600",
          )}
        >
          {bit}
        </span>
    ...
    >
      <div className="flex min-w-0 flex-1 flex-col gap-0.5">
        <span className="text-[10px] font-semibold tracking-widest text-zinc-400 uppercase dark:text-zinc-600">
          {label}
          {isInput && (
            <span className="ml-2 font-medium tracking-normal text-purple-500 normal-case dark:text-purple-400">
              input
            </span>
          )}
        </span>
        <span
          className={cn(
            ...
            empty ? "text-zinc-300 dark:text-zinc-700" : "text-zinc-900 dark:text-zinc-100",
          )}
        >
          {empty ? `—` : displayValue}
        </span>
      ...
      <div className="mt-1 flex shrink-0 items-center gap-1">
        <span className="font-mono text-[10px] font-semibold text-zinc-300 dark:text-zinc-700">
          {short}
        </span>
...
export function NumberBaseConverter() {
  ...
  const asciiPreview = useMemo(() => getAsciiPreview(result.decimalValue), [result.decimalValue]);
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      if (e.ctrlKey || e.metaKey) {
        ...
        if (e.key === "/") {
          e.preventDefault();
          document.getElementById("nbc-input")?.focus();
        }
      }
    };
    ...
    <div className="flex flex-col gap-8">
      ...
      <div className="flex flex-col gap-4">
        <div className="flex flex-wrap items-end gap-3">
          ...
          <div className="flex min-w-45 flex-1 flex-col gap-1.5">
            ...
            <input
              ...
              spellCheck={false}
              className="w-full rounded-lg border border-zinc-200 bg-zinc-50 px-3 py-2 font-mono text-sm text-zinc-900 placeholder:text-zinc-400 focus:ring-2 focus:ring-purple-500/40 focus:outline-none dark:border-zinc-800 dark:bg-zinc-900 dark:text-zinc-100 dark:placeholder:text-zinc-600"
            />
          </div>
          ...
          {!autoDetect && (
            <div className="flex flex-col gap-1.5">
              <label className="text-right text-xs font-semibold tracking-widest text-zinc-900 uppercase dark:text-zinc-500">
                Input base
              </label>
        ...
        <div className="flex flex-wrap items-center gap-4">
          ...
          <div className="relative">
            <select
              value={padLength}
              onChange={(e) => setPadLength(Number(e.target.value) as PadLength)}
          ...
          <div className="relative">
            <select
              value={groupStyle}
              onChange={(e) => setGroupStyle(e.target.value as GroupStyle)}
              className="cursor-pointer appearance-none rounded-md border border-zinc-200 bg-zinc-50 px-3 py-1.5 pr-8 text-xs font-medium text-zinc-700 focus:ring-2 focus:ring-purple-500/40 focus:outline-none dark:border-zinc-800 dark:bg-zinc-900 dark:text-zinc-300"
            >
          ...
          <label className="flex cursor-pointer items-center gap-1.5 select-none">
            <input
              type="checkbox"
              checked={showFormatDec}
              onChange={(e) => setShowFormatDec(e.target.checked)}
              className="h-3.5 w-3.5 accent-purple-600"
            />
            <span className="text-xs font-medium text-zinc-500 dark:text-zinc-400">
              Format decimal
        ...
        {result.error && input.trim() && (
          <p className="font-mono text-xs font-medium text-red-500 dark:text-red-400">
            {result.error}
          </p>
        )}
      </div>
      ...
      <Section title="Conversions">
        ...
        {hasResult && result.binary && result.binary.replace(/^-/, "").length <= 64 && (
          <div className="rounded-lg border border-zinc-200 bg-zinc-50 px-4 py-3 dark:border-zinc-800 dark:bg-zinc-900">
            <p className="mb-1 text-[10px] font-semibold tracking-widest text-zinc-400 uppercase dark:text-zinc-600">
              Bit visualization
            </p>
            <BitVisualizer
              binary={groupStyle !== "none" ? display(result.binary, 2) : result.binary}
            />
          </div>
        )}
        ...
        {asciiPreview && (
          <div className="flex items-center gap-3 rounded-lg border border-zinc-200 bg-zinc-50 px-4 py-3 dark:border-zinc-800 dark:bg-zinc-900">
            ...
            <span className="font-mono text-xs text-zinc-500 dark:text-zinc-400">
              U+
              {result.decimal
                ? parseInt(result.decimal).toString(16).toUpperCase().padStart(4, "0")
                : ""}
            </span>
          </div>
        )}
      ...
      <Section title="Character Converter">
        <div className="flex flex-wrap items-start gap-4">
          <input
            ...
            placeholder="A"
            maxLength={1}
            className="w-16 rounded-lg border border-zinc-200 bg-zinc-50 px-3 py-2 text-center font-mono text-lg text-zinc-900 focus:ring-2 focus:ring-purple-500/40 focus:outline-none dark:border-zinc-800 dark:bg-zinc-900 dark:text-zinc-100"
          />
          {charInfo && (
            <div className="flex flex-1 flex-wrap gap-3">
              {[
                ...
                <div
                  key={label}
                  className="flex flex-1 flex-col gap-0.5 rounded-lg border border-zinc-200 bg-zinc-50 px-3 py-2 dark:border-zinc-800 dark:bg-zinc-900"
                >
                  <span className="text-[10px] font-semibold tracking-widest text-zinc-400 uppercase dark:text-zinc-600">
                    {label}
                  </span>
      ...
      <Section title="IEEE 754 Float → Binary">
        <div className="flex flex-wrap items-start gap-4">
          <input
            ...
            placeholder="3.14"
            className="w-32 rounded-lg border border-zinc-200 bg-zinc-50 px-3 py-2 font-mono text-sm text-zinc-900 focus:ring-2 focus:ring-purple-500/40 focus:outline-none dark:border-zinc-800 dark:bg-zinc-900 dark:text-zinc-100"
          />
          {floatBinary && (
            <div className="flex flex-1 flex-col flex-wrap gap-3 md:flex-row">
              {[
                ...
                <div
                  key={label}
                  className="flex-1 rounded-lg border border-zinc-200 bg-zinc-50 px-3 py-2 dark:border-zinc-800 dark:bg-zinc-900"
                >
                  ...
                  <p className="mt-0.5 font-mono text-xs break-all text-zinc-700 dark:text-zinc-300">
                    {value}
                  </p>
                </div>
              ))}
      ...
      <Section title="Bitwise Playground">
        <div className="flex flex-wrap items-end gap-3">
          {/* Base for bitwise */}
          <div className="flex flex-col gap-1.5">
            <label className="text-[10px] font-semibold tracking-widest text-zinc-400 uppercase dark:text-zinc-600">
              Base
            </label>
            <div className="flex gap-1 rounded-lg border border-zinc-200 p-1 dark:border-zinc-800">
              {BASES.map(({ base, short }) => (
          ...
          <div className="flex w-full gap-2">
            <div className="flex min-w-0 flex-1 flex-col gap-1.5">
              ...
              <input
                ...
                placeholder="A"
                className="rounded-md border border-zinc-200 bg-zinc-50 px-3 py-1.5 font-mono text-xs text-zinc-900 focus:ring-2 focus:ring-purple-500/40 focus:outline-none dark:border-zinc-800 dark:bg-zinc-900 dark:text-zinc-100"
              />
            </div>
            {/* Op */}
            <div className="flex shrink-0 flex-col gap-1.5">
              ...
              <div className="relative">
                <select
                  value={bwOp}
                  onChange={(e) => setBwOp(e.target.value as BitwiseOp)}
                  className="cursor-pointer appearance-none rounded-md border border-zinc-200 bg-zinc-50 px-3 py-1.5 pr-8 font-mono text-xs font-medium text-zinc-700 focus:ring-2 focus:ring-purple-500/40 focus:outline-none dark:border-zinc-800 dark:bg-zinc-900 dark:text-zinc-300"
                >
            ...
            {bwOp !== "NOT" && bwOp !== "LSHIFT" && bwOp !== "RSHIFT" ? (
              ...
              <div className="flex min-w-0 flex-1 flex-col gap-1.5">
                ...
                <input
                  ...
                  className="w-20 rounded-md border border-zinc-200 bg-zinc-50 px-3 py-1.5 font-mono text-xs text-zinc-900 focus:ring-2 focus:ring-purple-500/40 focus:outline-none dark:border-zinc-800 dark:bg-zinc-900 dark:text-zinc-100"
                />
              </div>
            ) : null}
          </div>
        ...
        {bwResult && (
          <div className="mt-1 grid grid-cols-2 gap-2 sm:grid-cols-4">
            {(["binary", "octal", "decimal", "hex"] as const).map((k) => (
              ...
              >
                <span className="mb-0.5 block text-[10px] font-semibold tracking-widest text-zinc-400 uppercase dark:text-zinc-600">
                  {k}
                </span>
                <div className="flex items-center justify-between gap-1">
      ...
      <Section title="Arithmetic">
        <div className="flex flex-wrap items-end gap-3">
          ...
          <div className="flex w-full gap-2">
            ...
            <div className="flex shrink-0 gap-1 self-end rounded-lg border border-zinc-200 p-1 dark:border-zinc-800">
              {ARITH_OPS.map((op) => (
                <button
                  key={op}
                  onClick={() => setArOp(op)}
                  className={cn(
                    "w-8 rounded-md py-1 font-mono text-sm font-medium transition-colors",
                    arOp === op
                      ? "bg-purple-600 text-white"
                      : "text-zinc-500 hover:text-zinc-900 dark:text-zinc-400 dark:hover:text-white",
                  )}
      ...
      {hasResult && (
        <div className="flex flex-wrap items-center gap-2 border-t border-zinc-100 pt-2 dark:border-zinc-800">
          ...
          <span className="ml-auto hidden text-[10px] text-zinc-300 sm:block dark:text-zinc-700">
